...
void LookAtBehaviour::update( float step )
{
	...
	cameraPosition = glm::vec3(cameraPosition.x, cameraPosition.y + 0.57 + 0.3, cameraPosition.z + 0.9 + 2.46);	//Z should be adapted so it is at the right Z axis
	//eye is our camera position
	glm::vec3 eye = cameraPosition;                     	//the eye is the position of the camera
	//at is our target
	glm::vec3 at (target->getLocation() + glm::vec3(0.0 , 0.57 +  0.3  , 0.0));              		//looking at the target
