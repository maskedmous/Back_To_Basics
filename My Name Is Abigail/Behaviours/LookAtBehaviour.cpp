...
void LookAtBehaviour::update( float step )
{
	...
	cameraPosition = glm::vec3(cameraPosition.x, cameraPosition.y + 0.5, cameraPosition.z + 2.5);	//Z should be adapted so it is at the right Z axis
	//eye is our camera position
	glm::vec3 eye = cameraPosition;                     	//the eye is the position of the camera
	//at is our target
	glm::vec3 at (target->getLocation() + glm::vec3(0.0 , 0.5 , 0.0));              		//looking at the target
